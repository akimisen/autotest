...
from wtforms import StringField, PasswordField, SelectField, BooleanField, SubmitField, RadioField
from wtforms.validators import DataRequired,Length

class LoginForm(FlaskForm):
  user_name = StringField('用户名:', validators=[DataRequired(),Length(1,20)])
  password = PasswordField('密码:', validators=[DataRequired()])
  group_id = SelectField(validators=[DataRequired()], choices=((1,'组1'),(2,'组2'),(3,'组3')), default=1)
...
class RestCaseForm(FlaskForm):
  ...
  case_description = StringField('用例描述:', validators=[Length(1,100)])
  url = StringField('url:', validators=[Length(1,100)])
  method = RadioField(label='method:', choices=((1,'GET'),(2,'POST'),(3,'PUT'),(4,'DELETE')), default=1)
